  ...
  <Layout>
    ...
    <section className="relative py-32 overflow-hidden">
      ...
      <div className="container relative mx-auto px-4 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
          <span className="inline-block mb-6 rounded-full border border-secondary/30 bg-secondary/10 px-4 py-1.5 text-xs font-medium text-secondary">Development Services</span>
          <h1 className="font-heading text-4xl font-bold text-foreground sm:text-5xl lg:text-6xl">
            Build <span className="text-gradient-primary">Powerful</span> Digital Products
          </h1>
          ...
          <Button variant="hero" size="lg" className="mt-8">Start Your Project <ArrowRight className="ml-1 h-4 w-4" /></Button>
        </motion.div>
      </div>
    </section>
    ...
    <section className="py-24 bg-surface">
      <div className="container mx-auto px-4 lg:px-8">
        <SectionHeading badge="What We Offer" title="Full-Spectrum" highlight="Development" description="End-to-end development services covering every layer of the modern tech stack." />
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="mb-4 inline-flex rounded-lg bg-secondary/10 p-3"><s.icon className="h-6 w-6 text-secondary" /></div>
    ...
    <section className="py-24">
      <div className="container mx-auto px-4 lg:px-8">
        <SectionHeading badge="Tech Stack" title="Technologies We" highlight="Master" />
    ...
    <section className="py-24">
      <div className="container mx-auto px-4 lg:px-8">
        <SectionHeading badge="Solutions" title="Tailored for Your" highlight="Industry" description="We build solutions that fit your domain — from fintech to healthcare." />
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              className="rounded-xl border border-border bg-card p-6">
              <Workflow className="h-6 w-6 text-secondary mb-3" />
    ...
    <section className="py-24 bg-surface">
      <div className="container mx-auto px-4 lg:px-8 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <h2 className="font-heading text-3xl font-bold text-foreground md:text-4xl">Have a Project in <span className="text-gradient-primary">Mind?</span></h2>
          <p className="mt-4 text-muted-foreground max-w-xl mx-auto">Let's turn your idea into a production-ready product.</p>
          <Button variant="hero" size="lg" className="mt-8">Let's Talk <ArrowRight className="ml-1 h-4 w-4" /></Button>
        </motion.div>
      </div>
    </section>
  </Layout>
