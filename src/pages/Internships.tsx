...
const InternshipCard = ({ data }: { data: typeof devInternship }) => {
  const isSecondary = data.color === "secondary";
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
      className="rounded-xl border border-border bg-card p-8 lg:p-10">
      <div className={`mb-4 inline-flex rounded-lg p-3 ${isSecondary ? "bg-secondary/10" : "bg-accent/10"}`}>
        <data.icon className={`h-8 w-8 ${isSecondary ? "text-secondary" : "text-accent"}`} />
      </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {data.topics.map((t, i) => (
            <span key={i} className="flex items-center gap-2 text-sm text-muted-foreground">
              <CheckCircle className={`h-3.5 w-3.5 shrink-0 ${isSecondary ? "text-secondary" : "text-accent"}`} /> {t}
            </span>
          ))}
        </div>
      </div>
        ...
        <ul className="space-y-2">
          {data.outcomes.map((o, i) => (
            <li key={i} className="flex items-center gap-2 text-sm text-muted-foreground">
              <Award className={`h-3.5 w-3.5 shrink-0 ${isSecondary ? "text-secondary" : "text-accent"}`} /> {o}
            </li>
          ))}
        </ul>
      </div>

      <Button variant={isSecondary ? "hero" : "accent"} size="lg" className="w-full">
    ...
    <section className="py-24">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { icon: Users, title: "Expert Mentors", desc: "Learn from OSCP, CEH, and senior-level professionals." },
            { icon: BookOpen, title: "Hands-On Projects", desc: "Work on real client projects, not simulations." },
            { icon: Award, title: "Certification Support", desc: "We help you prepare for industry certifications." },
            { icon: ArrowRight, title: "Job Placement", desc: "Top performers get full-time offers at NxtGenSec." },
          ].map((item, i) => (
    ...
    <section className="py-24 bg-surface">
      <div className="container mx-auto px-4 lg:px-8 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <Button variant="hero" size="lg" className="mt-8">Apply Now <ArrowRight className="ml-1 h-4 w-4" /></Button>
        </motion.div>
      </div>
    </section>
  </Layout>
